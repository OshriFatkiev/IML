...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        min = f.compute_output(X=X, y=y)
        delta, sum, t = np.linalg.norm(f.weights), 0, 0

        while t < self.max_iter_ and delta > self.tol_:
            eta = self.learning_rate_.lr_step(t=t)
            temp = f.weights - eta * f.compute_jacobian(X=X, y=y)
            sum += temp
            delta = np.linalg.norm(temp - f.weights)
            if f.compute_output(X=X, y=y) < min:
                min = f.compute_output(X=X, y=y)
            f.weights = temp
            self.callback_(self, f.weights, f.compute_output(X=X, y=y), f.compute_jacobian(X=X, y=y), t, eta, delta)
            t += 1
